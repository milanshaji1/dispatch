...
1. **Honest evaluation.** The spike model is evaluated with a rolling-origin
   ...
   against two baselines any analyst would demand: climatological spike
...
Tests prove the hallucination gate: a seeded fabricated figure is detected
and blocks publication (`tests/test_verify.py`).
...
now pushes grid demand below zero on sunny days. The gate now encodes that
instead of the wrong assumption.
<!-- RESULTS:END -->
...
- **The verifier checks figures, not narrative.** A brief could cite correct
  ...
  structural prompt reduce but don't eliminate this. Case in point from the
  ...
  trading day (`n_intervals >= 285`), determined from the data rather than
  the clock.
